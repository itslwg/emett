...
approached a total of `r n_enrolled` patients for enrolment. Out of
these, `r n_after_excluding_missing_outcome` patients consented,
...
`r report("s30d", "Yes")` patients died within 30 days. We used
a temporal split to divide the cohort into a training and test
sample. The training sample included `r n_training_sample`
patients and the test sample `r n_test_sample` patients. The
AUROCCs of the priority levels assigned by the SuperLearner and
clinicians were `r superlearner_cat_auc_test` and
`r tc_auc_test` respectively. The difference in AUROCC was
`r diff_superlearner_cat_tc_auc`. The net reclassification in
events was `r nre` and in non-events `r nrne`.
...
We analysed data from `r n_after_excluding_missing_outcome` adult trauma
patients who presented to emergency departments at three public hospitals in
urban India. Out of these `r report("s30d", "Yes")` patients died from any
...
alternatives include random forests or convolutional neural networks.
...
The label used to train the ensemble model was all-cause 30 day
...
We then developed our ensemble model in the training set using the
SuperLearner R package [@SuperLearner]. SuperLearner is an ensemble
machine learning algorithm, meaning that it uses a library of
techniques or specific learners, in principle any technique or learner
that the analyst wants, to come up with an "optimal learner". Table
...
```{r superleaner-library, echo=FALSE}
tibble::tribble
(
...
)  %>% knitr::kable(caption = "Learners included in our SuperLearner library")

```

The ensemble model was then used to assign levels of priority to the
patients in the training set. This was done by binning the ensemble
...
clinicians. The performance of both the continuous ensemble model
prediction and the ensemble model priority levels in the training set
was then evaluated by estimating their AUROCC. We also visualised the
performance by plotting ROC and precision-recall curves.

### Comparing the ensemble model and Clinicians

We then used the ensemble model to predict the outcomes of the
patients in the test set and used the cutoff values from the training
...
During the study period, we approached a total of `r
round(n_enrolled*1.01)` patients for enrolment. A random sample of `r
round(n_enrolled*0.01)` observations were removed during data
de-identification. Consent was declined by `r n_enrolled -
n_after_excluding_no_consent` patients. Out of the `r
n_after_excluding_no_consent` patients who provided informed consent,
`r n_after_excluding_no_consent - n_after_excluding_missing_priority`
had missing data on priority level assigned by clinicians, leaving `r
n_after_excluding_missing_priority` patients. An additional `r
n_after_excluding_missing_priority -
n_after_excluding_missing_outcome` were excluded because of missing
outcome data. Thus, the final study sample included `r
n_after_excluding_missing_outcome` patients.

Table \@ref(tab:sample-characteristics) shows the characteristics of
our study sample. A total of `r report('n', strata = 'Missing
values')` patients had missing values in at least one feature. Among
the included patients the median age was `r report('age')` years. A
majority, `r report('sex', 'Male')` patients, were males. The most
common mechanism of injury was transport accidents, accounting for `r
report('moi', 'Transport accident')` patients. A total of `r
report('mot', 'Private vehicle')` patients were transported to
...
arrival to participating centres. Out of all patients, `r
report('s30d', 'Yes')` died within 30 days of arrival. The number of
patients in the training and test samples were `r n_training_sample`
and `r n_test_sample` respectively.

```{r sample-characteristics, echo=FALSE}
## Need to save table as data frame or equivalent before this will
## work
knitr::kable(table_of_sample_characteristics,
             caption = "Sample characteristics")
```

The AUROCC of the continuous ensemble model prediction in the training
sample was `r superlearner_con_auc_train`
(Fig. \@ref(fig:roc-plot)A). The cutpoints identified by the grid
search were `r optimal_breaks[1]`, `r optimal_breaks[2]`, and `r
optimal_breaks[3]`. We used these cutpoints to bin the continuous
...
the training sample was `r superlearner_cat_auc_train`.
Fig. \@ref(fig:prec-rec-plot)A shows the precision-recall curves in
the training sample.

```{r roc-plot, echo=FALSE,
fig.cap="Receiver operating characteristics curves in training (A) and test (B) samples"}
## Need to use ggplot2 to create these plots and save somewhere for
## this to work
roc.plot
```

```{r prec-rec-plot, echo=FALSE,
fig.cap="Precision-recall curves in training (A) and test (B) samples"}
## Need to use ggplot2 to create these plots and save somewhere for
## this to work
prec.rec.plot
```
...
the continuous ensemble model prediction was `r
superlearner_con_auc_test` (Fig \@ref(fig:roc-plot)B). The performance
...
the test sample was `r superlearner_cat_auc_test`. Fig
\@ref(fig:prec-rec-plot)B shows the precision-recall curves in the
test sample.
...
levels assigned by clinicians was `r tc_auc_test`. The difference in
AUROCC between the ensemble model priority levels and clinicians was
`r diff_superlearner_cat_tc_auc`. The net reclassification in events
and non-events were `r nre` and `r nrne` respectively. The overall
reclassification is shown in Table \@ref(tab:reclass-all)}.

```{r reclass-all, echo=FALSE}
knitr::kable(reclass.all, caption = "Reclassification measures")
```

Fig \@ref(fig:mortality.plot) shows that the number of patients
...
the green priority level to `r mortality_plot_data[1,4]` patients
whereas clinicians assigned this level to `r mortality_plot_data[9,4]`
patients. Among the patients that the ensemble model prioritised as
green `r mortality_plot_data[5,2]` died. The corresponding figure for
the clinicians was `r mortality_plot_data[13,2]`. In contrast, the
ensemble model assigned the yellow priority level to `r
mortality_plot_data[2,4]` patients, out of which `r
mortality_plot_data[6,2]` died. Corresponding figures for the
clinicians were `r mortality_plot_data[10,4]` and `r
mortality_plot_data[14,2]`.

```{r mortality-plot, echo=FALSE, 
fig.cap="Number of patients assigned to each priority level by the
SuperLearner and clinicians in the test sample. Percentages are
% with all cause 30-day mortality at each level"}
mortality.plot
```
